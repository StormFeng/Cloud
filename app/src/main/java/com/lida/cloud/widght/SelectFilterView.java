...
import com.midian.base.configlib.AppConstant;
...
public class SelectFilterView extends LinearLayout {
	...
	private void init(Context context) {
		...
		dividerPadding = dp2px(context, 6);
	...
	public void initTab(String[] tabs){
		this.tabs=tabs;
		tabCount= tabs.length;
		this.removeAllViews();
		for (int i = 0; i < tabs.length; i++)
    ...
    /**
     * 单独公开的方法，可以禁止某个tab的点击和改变字体和图标
     * @param position
     * @param isSelect
     */
    public void hidTabState(int position, boolean isSelect) {
...
//                    tab.setText(text);
	...
	private void addRightIcon(TextView tab, int i, boolean isUp) {
		...
		Drawable d = getResources().getDrawable(
				isUp ? R.drawable.icon_arrow_u : R.drawable.icon_arrow_d);
	...
	public void removeRightIcon(TextView tab, int i) {
		String text = tabs[i] + "   ";
		SpannableStringBuilder str = new SpannableStringBuilder(text);
		str.setSpan(null, text.length(), text.length(),
				Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		tab.setText(str);
	}
